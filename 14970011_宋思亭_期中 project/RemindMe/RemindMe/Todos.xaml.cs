...
namespace RemindMe
{
    ...
    public sealed partial class Todos : Page
    {
        ...
        protected override async void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            date.Date = Common.selectDate;
            ViewModel.selectDateItems(Common.selectDate);
            dataTransferManager.DataRequested += DataRequested;
        ...
        private void delete_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                using (var res = conn.Prepare(sql))
                {
                    res.Bind(1, item.ID);
                    res.Step();
                    ViewModel.getItems.Remove(item);
                }
            }
            catch (Exception err)
            {
                ...
                if (createButton.Content.ToString() == "Create")
                {
                    try
                    {
                        ...
                        using (var res = conn.Prepare(sql))
                        {
                            ...
                            res.Bind(6, "false");
                            res.Step();
                            ViewModel.AddItem(conn.LastInsertRowId(), date.Date.Date, time.Time, Common.selectName, title.Text, details.Text, false);
                            Common.selectName = "";
